...
RACINE_CIBLE=tiles_croisement_241107
...
FROM ( SELECT osm_id,
              ...
              nom_commune_fin,
              code_insee_fin,
              (plus_petit_chevauchement < $d)::boolean AS ppc,
              ST_AsMvtGeom(
                geometrie_osm_3857,
                BBox($tx, $ty, $tz),
                4096,
...
{
  psql -d bano -U cadastre -tq -c "$(cvl_voie $z $x $y $d)" | xxd -r -p ;
  psql -d bano -U cadastre -tq -c "$(cvl_point_debut $z $x $y $d)" | xxd -r -p ;
  psql -d bano -U cadastre -tq -c "$(cvl_point_fin $z $x $y $d)" | xxd -r -p ;
  psql -d bano -U cadastre -tq -c "$(cvl_point_croisement $z $x $y $d)" | xxd -r -p ;
} > $ofile
echo $ofile
